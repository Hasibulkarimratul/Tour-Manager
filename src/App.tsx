...
function TourCard({
  ...
  const longPress = useLongPress(() => handleLongPress(), 700);

  return (
    <div
      onClick={isConfirmingDelete || showOptions ? undefined : onClick}
      {...longPress}
      className={`item-card p-6 rounded-[32px] hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer group flex justify-between items-center relative overflow-hidden ${isHighlighted ? "ring-4 ring-purple-500 ring-offset-4 dark:ring-offset-slate-950 scale-105" : ""} ${isConfirmingDelete ? "bg-red-500/10 border-red-500" : ""} ${showOptions ? "bg-purple-500/10 border-purple-500" : ""}`}
    ...
    >
        ...
        {showOptions ? (
          <div className="flex gap-2 w-full pr-12">
            <button 
              ...
              className="flex-1 bg-amber-500 hover:bg-amber-600 text-white font-black uppercase tracking-widest text-[10px] py-2 rounded-xl transition-all"
            >
              {tour.isArchived ? "Restore" : "Archive"}
            </button>
            <button
               onClick={triggerDelete}
               className="flex-1 bg-red-500 text-white font-black uppercase tracking-widest text-[10px] py-2 rounded-xl"
            >
               Delete
            </button>
          </div>
          ...
          <>
            <h3
              className={`text-xl font-black transition-colors uppercase tracking-tight truncate ${isConfirmingDelete ? "text-red-500" : tour.isArchived ? "text-amber-500 group-hover:text-amber-400" : "group-hover:text-purple-500"}`}
            >
              {isConfirmingDelete ? (isAdmin ? (tour.isArchived ? "Confirm Permanent Delete?" : "Confirm Delete?") : "Confirm Leave?") : tour.name}
              {tour.isArchived && <span className="ml-2 text-[10px] bg-amber-500/10 text-amber-500 px-2 py-0.5 rounded-full inline-block align-middle">Archived</span>}
            </h3>
            <div
              className={`flex flex-wrap gap-x-4 gap-y-1 mt-1 text-xs font-bold ${isConfirmingDelete ? "text-red-400" : "text-[var(--text-muted)]"}`}
            >
      ...
      {!showOptions && (
        <div className={`flex items-center gap-2 shrink-0 transition-colors opacity-100 z-10 ${isConfirmingDelete ? "opacity-100" : "sm:opacity-0 sm:group-hover:opacity-100"}`}>
          {tour.isArchived && isAdmin && onArchive && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                onArchive(e);
              }}
              className="p-3 text-[var(--text-muted)] hover:text-amber-500 rounded-full transition-colors bg-[var(--bg-surface)] border border-[var(--border-color)] shadow-sm"
              title="Restore Tour"
            >
              <Undo2 size={18} />
            </button>
          ...
          <button
            ...
            className={`p-3 transition-colors rounded-full ${isConfirmingDelete ? "bg-red-500 text-white" : "text-[var(--text-muted)] hover:text-red-500 bg-[var(--bg-surface)] border border-[var(--border-color)] shadow-sm"}`}
...
export default function App() {
  ...
  const handleToggleArchiveTour = async (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    const tourToArchive = tours.find(t => t.id === id);
    if (!tourToArchive || tourToArchive.adminId !== user?.uid) return;
    
    updateDoc(doc(db, 'tours', id), { isArchived: !tourToArchive.isArchived }).catch(e => console.error(e));
  };
      ...
      <AnimatePresence mode="wait">
        {!activeTourId ? (
          ...
          >
            <header className="mb-12 flex flex-col md:flex-row justify-between items-center md:items-start text-[var(--text-main)] relative gap-4">
              ...
              <div className="flex items-center gap-3 mt-2 md:mt-1">
                ...
                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all ${isOnline ? 'bg-green-500/5 text-green-600 border-green-500/20' : 'bg-amber-500/5 text-amber-600 border-amber-500/20'}`}>
                  ...
                  <button
                    onClick={() => setShowArchivedTours(!showArchivedTours)}
                    title="Archived Vault"
                    className={`p-3 transition-all rounded-full border shadow-sm ${showArchivedTours ? 'bg-amber-500 text-white border-amber-500 hover:bg-amber-600' : 'text-[var(--text-muted)] hover:text-amber-500 bg-[var(--bg-surface)] border-[var(--border-color)] hover:border-amber-500'}`}
                  >
                    <History size={18} />
                  </button>
                  <button
            ...
            <div className="space-y-4">
              ...
              {(() => {
                ...
                const archivedTours = tours.filter(t => t.isArchived);
                
                if (showArchivedTours) {
                  return (
                    <div className="flex flex-col gap-4">
                      <h2 className="text-xs font-black uppercase tracking-widest text-[var(--text-muted)] border-b border-[var(--border-color)] pb-2 flex items-center justify-between">
                        <span>Archived Trips</span>
                        <span className="bg-[var(--bg-surface)] border border-[var(--border-color)] px-2 py-0.5 rounded-full text-[10px]">{archivedTours.length}</span>
                      </h2>
                      {archivedTours.length === 0 ? (
                        ...
                        archivedTours.map((tour) => (
                           <div key={tour.id} className="opacity-80 saturate-50 hover:opacity-100 hover:saturate-100 transition-all">
                             <TourCard
                                tour={tour}
                                highlightId={highlightId}
                                onClick={() => setActiveTourId(tour.id)}
                                onDelete={(e) => handleDeleteTour(tour.id, e)}
                                onArchive={(e) => handleToggleArchiveTour(tour.id, e)}
          ...
          >
            ...
            {activeTab === "expenses" && !activeTour.isArchived && (
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[var(--bg-surface)] p-4 rounded-3xl border border-[var(--border-color)] shadow-sm">
        ...
        ) : activeTour.whatsappGroupLink ? (
          <a
            href={activeTour.whatsappGroupLink}
            target="_blank"
            rel="noopener noreferrer"
            className="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all bg-[#25D366] text-white hover:bg-[#20bd5a] shadow-lg shadow-[#25D366]/30"
          >
            Join
          </a>
        ) : null}
      ...
      {!activeTour.isArchived && (isAdmin || (editingMemberId && editingMemberId === claimedMemberId)) && (<div className="item-card rounded-[32px] p-6 shadow-xl border-purple-500/20">
        ...
        >
          {activeTour.members.map((m) => (
            <MemberCard
              ...
              isAdmin={isAdmin && !activeTour.isArchived}
              isMemberAdmin={m.id === activeTour.adminId}
              isSelf={!activeTour.isArchived ? (m.id === claimedMemberId) : false}
            />
          ))}
        </div>
      </div>
...
function ExpenseCard({
  ...
  return (
    <>
      <div
        id={`expense-${exp.id}`}
        className={`group relative bg-[var(--bg-surface)] border rounded-[24px] p-5 hover:border-purple-500/50 hover:shadow-xl transition-all ${!activeTour.isArchived ? 'cursor-pointer' : 'cursor-default'} overflow-hidden ${isValid ? "border-[var(--border-color)]" : "border-red-500 border-2 bg-red-500/5"} ${isHighlighted && isValid ? "ring-2 ring-purple-500 scale-102 bg-purple-50/10" : ""}`}
        onClick={() => { if (!activeTour.isArchived) onEdit(exp); }}
      >
        {!isValid && (
        ...
        {isValid && !activeTour.isArchived && (isAdmin || exp.creatorId === currentUserId) && (
          <button
